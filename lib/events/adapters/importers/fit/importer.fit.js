...
var EventImporterFIT = /** @class */ (function () {
            ...
            fitFileParser.parse(arrayBuffer, function (error, fitDataObject) {
                ...
                var activities = fitDataObject.activity.sessions.map(function (sessionObject) {
                    ...
                    if (!activity.getLaps().length) {
                        fitDataObject.records.forEach(function (sessionLapObjectRecord) {
                            var point = _this.getPointFromSessionLapObjectRecord(sessionLapObjectRecord);
                            activity.addPoint(point);
                        });
                    }
    ...
    EventImporterFIT.getPointFromSessionLapObjectRecord = function (sessionLapObjectRecord) {
        ...
        // Add Distance
        if (event_utilities_1.isNumberOrString(sessionLapObjectRecord.distance)) {
            point.addData(new data_distance_1.DataDistance(sessionLapObjectRecord.distance));
        }
        // Add HR
        ...
        // Add Form Power
        if (event_utilities_1.isNumberOrString(sessionLapObjectRecord['Form Power'])) {
            point.addData(new data_form_power_1.DataFormPower(sessionLapObjectRecord['Form Power']));
        }
        // Add Leg Stiffness
        if (event_utilities_1.isNumberOrString(sessionLapObjectRecord['Leg Spring Stiffness'])) {
            point.addData(new data_leg_stiffness_1.DataLegStiffness(sessionLapObjectRecord['Leg Spring Stiffness']));
        }
        // Add Vertical  Oscillation
        if (event_utilities_1.isNumberOrString(sessionLapObjectRecord.vertical_oscillation)) {
            point.addData(new data_vertical_oscillation_1.DataVerticalOscillation(sessionLapObjectRecord.vertical_oscillation));
        }
        return point;
    };
    ...
    EventImporterFIT.getStatsFromObject = function (object) {
        ...
        if (event_utilities_1.isNumberOrString(object.total_training_effect)) {
